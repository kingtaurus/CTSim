function [ x, phis] = reconPsrSpectrumBinningNesterovSqs( sino, ...
...
fprintf('\n\n Polychromatic Statistical Reconstruction using %d - Spectrum Binning using Nesterov SQS: ', length(Phis));
tRecon = tic;
...
x = E .* rho ;
% curv = E .* At( A( E ) .* sino );
curv = (fphi.^2 +  ftheta.^2) .* At( A( ones( size(E ), 'single' ) ) .* sino );
...
for itn = 1 : itnlim
    ...
    for isub = 1:numos
        ...
        %estimate sinogram
        [sphi, stheta] = A2os( fphi .* z, ftheta .* z, isub );
        ...
        drho =  ( - N + beta * E.* S( E.* z ) ) ./ (curv + beta * E.* T( E.* z ));
