...
#include "cbct_dd_back.c"
#include "cbct_dd_proj.c"

#define USE_CUDA_DEVICE TRUE
...
static bool cbct_proj_all
	...
	Cmx mx_map){
		...
		float *proj;
		bool *map;        
        const int* dim_image = mxGetDimensions( mx_image );
...
#if USE_CUDA_DEVICE
    sof = cbct_proj_dd_gpu(  nx, ny, nz, dx, dy, dz, offset_x, offset_y, offset_z, nu, nv, du, dv, offset_u, offset_v, sad, add, sdd, noviews, betas, couchz, detector_type, image, proj, map  );
#else
    sof = cbct_proj_dd_cpu(  nx, ny, nz, dx, dy, dz, offset_x, offset_y, offset_z, nu, nv, du, dv, offset_u, offset_v, sad, add, sdd, noviews, betas, couchz, detector_type, image, proj, map  );
...
#if USE_CUDA_DEVICE
    sof = cbct_back_dd_gpu(   nx, ny, nz, dx, dy, dz, offset_x, offset_y, offset_z, nu, nv, du, dv, offset_u, offset_v, sad, add, sdd, noviews, betas, couchz, detector_type, image, proj, map);
#else
    sof = cbct_back_dd_cpu(   nx, ny, nz, dx, dy, dz, offset_x, offset_y, offset_z, nu, nv, du, dv, offset_u, offset_v, sad, add, sdd, noviews, betas, couchz, detector_type, image, proj, map);
#endif
