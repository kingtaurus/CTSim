...
/*
 * 
 * distance driven forward projection
 *
 *
 */
			...
			for ( iy = 0; iy < ny ; iy ++ ) {
				...
				for ( ix = 0; ix < nx ; ix++ ) {
					...
					for ( iz = iz_start ; iz < iz_stop ; iz++ ){
						...
						voxel = ampu * ampv *  image[iz*nxy+iy*nx+ix];
						...
						for ( iu = iumin; iu <= iumax; iu++ ){
							for ( iv = ivmin; iv <= ivmax; iv++ ){
								proj[ib*nu*nv+iu*nv+iv] = proj[ib*nu*nv+iu*nv+iv] +
									weights_u[iu-iumin] * weights_v[iv-ivmin] * voxel;
							}//iv
...
/*
 *
 * trapesoid footprint projection 
 *
 *
 */
bool cbct_proj_tf(
