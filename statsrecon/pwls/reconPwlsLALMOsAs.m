function [x, phis, rmsds] = reconPwlsLALMOsAs( y, w, geom, beta, pfun, itnlim, delta, numos, imgOpt  )
...
fprintf('Reconstructing with single energy data using PWLS LALM OS AS... \n');
tRecon = tic;
...
Aos     = @( x, is, map )forwardProject1Mex( x, geom, numos, is, 'proj,dd', map );
Atos    = @( x, is, map )backProject1Mex( x, geom, numos, is, 'back,dd', map );
A       = @( x )forwardProject1Mex( x, geom, 1, 0, 'proj,dd', geom.map );
At      = @( x )backProject1Mex( x, geom, 1, 0, 'back,dd', geom.map );
...
for itn = 1 : itnlim
    ...
    for isub = 1 : numos
        ...
        r = Aos( x, isub, geom.map ) - Os( y, isub );
        ...
        for jsub = 1:numas
            ...
            if jsub > 1
                r = r +  Aos( d, isub, maps{jsub - 1} ) ;
            end

            
            dL = Atos( Os( w, isub) .* r, isub, maps{jsub} ) ;
            ...
            %s = updateImageAxialBlocks( s, zeros( size(x), 'single'),  maps{jsub}, 0 );
            ...
            x = updateImageAxialBlocks( x, xold, maps{jsub}, 0 );
            d = x - xold;
