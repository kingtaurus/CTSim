...
bool cbct_back_pd(
    ...
    float xc, yc, sdd, camera_x, camera_y;
    float u0, v0, f0;
    float w2s, u, v, wul, wvl;
    ...
    for ( ib = 0; ib < noviews ; ib++) {
        ...
        for ( iy = 0; iy < ny ; iy ++ ) {
            ...
            for ( ix = 0; ix < nx ; ix++ ) {
                ...
                f0 = xc * pm[2] + yc * pm[5] + wz * dz * pm[8] + pm[11];
                
                w2s = sdd * sdd / ( ( camera_x - xc ) * ( camera_x - xc ) + ( camera_y - yc ) * ( camera_y - yc ));
                
                //mexPrintf("here: u0 = %f, v0 = %f \n", u0, v0);
                
                for ( iz = 0; iz < nz ; iz ++ ){
                    
                    u = ( u0 + iz * dz * pm[6] ) / ( f0 + iz * dz * pm[8] );
                    v = ( v0 + iz * dz * pm[7] ) / ( f0 + iz * dz * pm[8] );
