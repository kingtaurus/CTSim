function [sinoRaw, sinoPrime, sinoScatter] = combineScatterSimulationNew( sinoRaw, ...
...
for iv = 1 : size( sinoScatter, 3) - 1
    ...
    for isubv = 1 : veiwDownSampleRate
        ...
        if size( viewRaw, 1 ) == size( viewPrime, 1 )
            ...
            %******************* For only using 1/3 of the detector
%             scale = scale/3.;   
        end
        ...
        if scatterCorrection
            viewScatter = poissrnd( viewScatter * scale );
            viewScatter1 = ScatterCorr(viewScatter);  % Apply scatter estimation
            
           
            
%             figure(11)
%             imshow( log10( viewScatter1 + 1  ), [0 5]);
%             
              viewScatter1 = min( viewScatter1, 0.9 *( viewRaw + viewScatter ) );
%             figure(12)
%             imshow( log10( viewScatter1 + 1  ), [0 5]);
%             
%             figure(21)
%             imshow(  log10( medfilt2( viewRaw + viewScatter - viewScatter1, [3 3] ) ), [0 5]);
%             
%             pause(0.2);
            
            sinoRaw( :, :, (iv-1)* veiwDownSampleRate + isubv ) = viewRaw + viewScatter - viewScatter1;
