function img = backProjectDistanceDriven2(sino, geom, isFlatDetector)
...
for view = 1:noViews
    ...
    for ix = 1 : nx
        ...
        for iy = 1 : ny
            ...
            if (isFlatDetector == 1) % Flat Panel
                [umin, umax] = boundariesDistanceDrivenFlat(dsd_ds, ...
                    tproj0, dsxy0, wu, dx_cos_2, dx_sin_2, dy_sin_2, n_dy_cos_2, alongx);
            elseif (isFlatDetector == 0) % Standard Curved Panel
                [umin, umax] = boundariesDistanceDrivenArc(dsd_ds, ...
                    tproj0, dsxy0, wu, dx_cos_2, dx_sin_2, dy_sin_2, n_dy_cos_2, alongx);
            elseif (isFlatDetector == 2) % Special Arc Panel
                [umin, umax] = boundariesDistanceDrivenArc2(geom.sad, geom.add, du, ...
                    tproj0, dsxy0, wu, dx_cos_2, dx_sin_2, dy_sin_2, n_dy_cos_2, alongx);                
            end
