function [xs, phis, regs] = reconPwlsTruePathSeekingSelf( y, w, geom, pfun, delta, numos, img0, img1, dv, updateRate, noFrames )
...
fprintf('Ture path seeking (TPS-2) for PWLS method with updata rate %0.3f%% ... \n', updateRate);
tRecon = tic;
...
while totalAbsDistance >= saveThreshold && iframe <= noFrames
    ...
    for isub = 1 : numos
        ...
        gradientPenaltyAcc = gradientPenaltyAcc + gradientPenalty;
        
        % using the KKT condition to estimate the tuning parameter value
        if isub == numos || betap < 10
            
            gradientRatio   =  - gradientWLS ./ gradientPenalty;
            valid           = validPixelsAll & ~isnan( gradientRatio ) & ~isinf( gradientRatio ) & abs(gradientPenalty) > 1e-8;
            
            beta =  median( gradientRatio(valid(:)) );
            % get a little extreme
            if forwardPathSeeking
                betap = max( 1.1 * beta, betap );
            else
                betap = min( 0.9 * beta, betap );
            end
        end
