function img = reconHelicalFDK( sino, geom, H, noViewsPerSlice, tiltedFilter )
% function img = reconHelicalFDK( sino, geom, window, segmentLength, tiltedFilter, crop )
%
...
if noViewsPerSlice ~= geom.noViewsTurn
    fprintf('\tWarning: FDK method is only implemented for 2 pi reconstruction');
end

dbeta = abs(geom.betas(end) - geom.betas(1)) / (noViews-1);

%geometery parameter for a slice
geomSlice               = geom;
geomSlice.reconSize(3)  = 1;

% cosine weighting and ramp filtering the sinogram
sino = cosineWeighting( sino, geom );
sino = filterHelicalSinogram( sino, H, 0.5, geom, tiltedFilter );
...
for iz = 1: nz
    ...
    sinoSlice = sino(:,:,  iview_start : iview_stop);
