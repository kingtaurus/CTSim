function [sino] = sinogramGainCorrectionSegmentationPrior( sino, geom, imgPrior, secondSinoIndex, threshold )
...
fprintf('\tSegmentation level = %2.3f \n', level );
imgBW = single(imgPrior > level);
thicknessProjections = forwardProjectMex( imgBW,geom );
...
if ndims( sino ) == 3
    ...
    redundantThickness = zeros( numberOfRedundentPixels * k, 1);
    
    for i = 0:k-1
        centralRow = squeeze( sino(end/2 + i - (k-1)/2,:,:));
        centralRowThickness = squeeze( thicknessProjections(end/2 + i - (k-1)/2,:,:));
        
        redundantPixel1(i*numberOfRedundentPixels+1 : (i+1)*numberOfRedundentPixels) = centralRow( redundantSino1(:) );
        redundantThickness(i*numberOfRedundentPixels+1 : (i+1)*numberOfRedundentPixels) = centralRowThickness( redundantSino1(:) );
    ...
    redundantThickness = thicknessProjections( redundantSino1(:) );
...
redundantThickness = redundantThickness( validPixels );
...
X = [ redundantPixel2 redundantThickness ones(n, 1) ];

p = ( X' * X )^(-1) * X' * redundantPixel1;
fprintf('\tModel 1: sino1 = %2.2f * sino2 +  %2.2f * thickness +  %2.2f \n', p(1), p(2), p(3)  );

residues = redundantPixel1 - ( p(1) * redundantPixel2 + p(2) * redundantThickness + p(3) );
sigma =  sqrt( sum( residues.^2 ) / (n-1));
for i = 1:3
    z = n * p(i) / sigma * std( X(:,i));
    fprintf('\tp(%i) = %2.3f, \tt-test = %2.3f, \tp-val = %2.3e. \n', i,  p(i), z, 1-tcdf(abs(z), n-3) );
end
...
% figure;
% subplot(221);
% plot(redundantPixel2, redundantPixel1, '.' ); xlabel 'mv', ylabel 'km';
% subplot(222);
% plot(redundantThickness, redundantPixel1, '.' ); xlabel 'thickness', ylabel 'kv';
% subplot(223);
% plot(redundantThickness, redundantPixel1 - redundantPixel2 * p(1) - p(3), '.' ); xlabel 'thickness', ylabel 'subtract';
% subplot(224);
...
if ndims( sino ) == 3
    sino(:,:,secondSinoIndex:end) = p(1) * sino(:,:,secondSinoIndex:end) + p(2) * thicknessProjections(:,:,secondSinoIndex:end) + p(3);
else
    sino(:,secondSinoIndex:end) = p(1) * sino(:,secondSinoIndex:end) + p(2) * thicknessProjections(:,secondSinoIndex:end) + p(3);
end
